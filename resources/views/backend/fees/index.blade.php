...
@section('content')
    <div class="roles-permissions">
        ...
        <input type="text" id="searchInput" placeholder="Search..." class="appearance-none border rounded-md w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        <table class="mt-8 bg-white rounded border-b-4 border-gray-300" style="width: 100%">
            <thead>
            <tr class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                <th class="w-3/12 px-4 py-3">Name</th>
                <th class="w-3/12 px-4 py-3">Email</th>
                <th class="w-2/12 px-4 py-3">Class</th>
                <th class="w-2/12 px-4 py-3">Phone</th>
                <th class="w-2/12 px-4 py-3 text-right">Action</th>
            </tr>
            </thead>
            <tbody id="tableBody">
            @foreach ($students as $student)
                <tr class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <td class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->user->name }}</td>
                    <td class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->user->email }}</td>
                    <td class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->class->class_name ?? '' }}</td>
                    <td class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->phone }}</td>
                    <td class="w-2/12 flex items-center justify-end px-3">
